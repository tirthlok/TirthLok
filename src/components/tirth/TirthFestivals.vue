<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-3 mb-8">
      ...
      <h2 class="text-lg sm:text-xl font-bold bg-gradient-to-r from-red-600 via-rose-600 to-red-700 bg-clip-text text-transparent">Festivals & Events</h2>
    </div>
    ...
    <div v-if="tirth.festivals.length === 0" class="text-center py-8 md:py-12 bg-gradient-to-r from-gray-50 to-yellow-50 rounded-2xl border-2 border-dashed border-gray-300">
      ...
      <p class="text-gray-600 font-semibold text-sm">No festivals recorded</p>
          ...
          <div class="flex items-start gap-5">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-start justify-between gap-4 mb-3 flex-wrap">
                <h3 class="font-bold text-base md:text-lg text-gray-900 group-hover:text-amber-700 transition-colors">
                  {{ festival.name }}
                </h3>
                <span class="px-4 py-1.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold rounded-full shadow-md whitespace-nowrap">
                  {{ festival.month }}
                </span>
              </div>
              ...
              <div v-if="festival.specialEvent" class="p-4 bg-gradient-to-r from-red-100 to-pink-100 border-l-4 border-red-500 rounded-lg group-hover:shadow-md transition-all">
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-sm font-semibold text-red-800 leading-relaxed">{{ festival.specialEvent }}</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div v-if="tirth.festivals.length > 0" class="mt-8 p-6 bg-gradient-to-r from-amber-100 to-yellow-100 rounded-2xl border-2 border-amber-300">
      <div class="flex items-start gap-4">
        ...
        <div>
          <h4 class="font-bold text-sm text-amber-900 mb-2">Festival Information</h4>
