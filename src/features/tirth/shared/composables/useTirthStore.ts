...
import { useTirthApi } from '~/features/tirth/tirthpage/services/tirthApi'
  ...
  actions: {
    async fetchTirths(page = 1) {
      ...
      try {
        ...
        const response = await $fetch<{ success: boolean; data: Tirth[]; pagination: { total: number; page: number; pages: number } }>(`/api/tirth?${params.toString()}`)
